...
export class GameScreenView implements View {
    ...
    private expressionText: Konva.Text;
    private answerButtons: Konva.Group[] = [];
    ...
    updateExpression(expression: { 
        ...
        operator: string;
    }): void { //parameters for updateExpression function
    ...
    updateAnswerChoices(choices: Array<{ numerator: number; denominator: number }>): void { //takes in array of answer choices
    ...
    flashFeedback(isCorrect: boolean, buttonIndex: number): void {
        ...
        const rect = button.findOne('Rect');//finds konva rectangle inside the button
        
        if (rect) { //only proceed if rectangle exists
